...
float maxX(float t[],int n){//*fonction qui retourne la plus grande moyenne*//
    int i;
    float max_note;
    max_note=t[0];
    for(i=0;i<n;i++){
...
int moyenne(float t[],int n){//*fonction qui retourne la moyenne sup�rieur ou �gale 10 *//
    int i,nb=0;
    for(i=0;i<n;i++){
        if(t[i]>=10){nb++;}
    }
...
void tri(float t1[], float t2[],int n,int m){//*proc�dure qui tri des notes par ordre croissant de deux classes*//
int i,temp,j,k;
float c[n+m];
k=n-1;
